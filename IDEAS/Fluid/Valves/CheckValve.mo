...
model CheckValve
  "Check valve or non-reversal valve: allows only flowrate from a to b"
  extends IDEAS.Fluid.BaseClasses.PartialResistance(
    final linearized = true,
...
equation
  ...
  if computeFlowResistance then
    if linearized then
      m_flow*m_flow_nominal_pos = max(k^2*dp, 0);
    else
